...
def test_missing_citation_is_not_auto_attached():
    ...
    citations, is_clean, meta = validate_citation_references(raw_answer, hits)
    assert citations == []
...
def test_invalid_citation_causes_validation_failure():
    ...
    citations, is_clean, meta = validate_citation_references(hallucinated_answer, hits)
...
def test_client_cannot_override_production_retrieval_policy():
    """Production QueryIn chỉ chấp nhận question và user_groups, từ chối tham số can thiệp pipeline."""
    payload_data = {
...
def test_unauthorized_document_never_reaches_retrieval():
    ...
    retriever.chunks = [
        {"chunk_id": "c1", "text": "Quy chế an ninh", "source": "security.txt", "security_scope": ["public", "employee"]},
        {"chunk_id": "c2", "text": "Bảng lương tuyệt mật", "source": "salary.txt", "security_scope": ["confidential", "hr"]},
    ]
    ...
    retriever.index.search.return_value = (np.array([[0.8, 0.9]], dtype="float32"), np.array([[0, 1]], dtype="int64"))
    ...
    assert "c2" not in retrieved_ids, "LỖI BẢO MẬT ACL: Tài liệu bí mật lọt vào retrieval của user không có thẩm quyền!"
